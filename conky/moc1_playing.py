...
def Header(info):
    ...
    if info['State'] == "PAUSE":
        shift = ""
    header = "MOC  %s ${alignr 100}${font PizzaDude Bullets:size=10}x${font} ${mixerbar}" % info['State']
    return header
...
def TimeBar(info):
    ...
    time = "${voffset 5}${color green}%s${color} - %s  ${execbar echo %d}" % (info['TimeLeft'], info['TotalTime'], percent)
    return time
